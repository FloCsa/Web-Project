...
class User extends Database
{
	...
	{
		$sql = "SELECT `email`,`passwort` FROM `nutzer` WHERE `email`='" . $this->escapeString($username) . "'";
	...
	{
		$id = intval($id);
		$sql = "SELECT * FROM `nutzer` WHERE `email`=".$id;
	...
	public static function existsWithUsername($username)
	{
		...
		$sql = "SELECT COUNT(`email`) AS num FROM `nutzer` WHERE `email`='".$db->escapeString($username)."'";
		$result = $db->query($sql);
	...
	public static function createUser($data)
	{
		...
		$sql = "INSERT INTO `nutzer`(`email`,`passwort`) VALUES('".$username."','".$password."')";
		$db->query($sql);
	}
